#Calculate percentage of coexpressed interactions across different tumor types
...
correlation.count <-function(file, filename){
	...
	#any NA is assigned as 1 b/c it is giving 0 pvalue to highly correlated genes and when you take log it turns to 0
	file$V3[is.infinite(file$V3)] <- 1
	file$V3[is.na(file$V3)] <- 1 
	file$V4 <- -log10(file$V3)
	#assign NA, Inf to 0
	#file$V4[is.infinite(file$V4)] <- 0 
	#file$V4[is.na(file$V4)] <- 0 
	#count if pvalue is more than 1.2
	corr.sum <- sum(file$V4>2)
...
correlation.files <- Sys.glob("*.int_2.5")
...
sign.corr.int <- sapply(corr.value,function(x) (x*100)/1458)   #1458
